...
```yaml
task: {taskIdentifier()}
responsible: {AgentName}          # Agent executing this task (e.g., Dex, Quinn, Pax)
executor_type: Agente          # Open-source: always "Agente" (Worker/Humano/Clone for services only)
...
- campo: {fieldName}
  ...
  required: {true|false}
  default: {defaultValue}          # Optional: default if not provided
...
function generateErrorMessage(agent, errorType, errorDetails) {
  ...
  const templates = {
    ...
    Guardian: "⚠️ Validation falhou. Bloqueando até resolução.",
