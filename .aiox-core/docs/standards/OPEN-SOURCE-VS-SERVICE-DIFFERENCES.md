...
**Open-Source Rule:**
- Always use `executor_type: Agente`
...
function validateTask(task) {
  const required = ['task', 'responsible', 'executor_type', 'Input', 'Output'];

  // Open-source specific: executor_type must be "Agent" (except MMOS)
  if (task.executor_type !== 'Agent' && !task.isMmosSquad) {
    console.warn(`Open-source tasks should use executor_type: Agent. Found: ${task.executor_type}`);
  }
...
- [ ] Change `executor_type: Worker` → `executor_type: Agente`
- [ ] Change `executor_type: Humano` → `executor_type: Agente`
- [ ] Change `executor_type: Clone` → `executor_type: Agente` (unless MMOS)
...
- [ ] Keep `executor_type: Agente` OR change based on EXECUTOR-DECISION-TREE.md
...
|---------|-------------|---------|
| **executor_type** | Agent only | Agente/Worker/Humano/Clone |
