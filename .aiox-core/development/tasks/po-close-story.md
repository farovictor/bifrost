...
- campo: story_path
  ...
  source: User Input
  required: true
  validation: Must be valid story file path

- campo: pr_number
  type: number
  source: User Input
  required: false
  validation: Valid PR number if provided

- campo: commit_sha
  type: string
  source: User Input
  required: false
  validation: Valid git SHA (7+ chars)
...
- campo: story_updated
  type: boolean
  destination: Story file
  persisted: true

- campo: epic_updated
  type: boolean
  destination: Epic index file
  persisted: true
