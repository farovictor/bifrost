...
- campo: workflow_state
  type: object
  destination: File system (.aiox/{instance-id}-engine-state.yaml)
  persisted: true
...
3. For each field in `Input` with `origem: User Input` and `required: true`:
